...
public class BattleStone : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        {
            battleui.passiveActive = true;
        }
        else if (transform.tag == "ma" && count <= 2)
        {
            battleui.passiveActive = false;
        }

        if (photonView.IsMine && battleui.battleStone == null)
        {
            battleui.battleStone = this;
            battleui.number = 1;

            if (transform.tag == "cha" || transform.tag == "po") battleui.passiveActive = true;
        }
        if (this.photonView.IsMine)
        {
            if (battleui.number != 1)
            {
                battleui.number = 1;
            }
    ...
    private void RotateToMouse()
    {
        ...
        if (plane.Raycast(ray, out float distance))
        {
            ...
            transform.rotation = Quaternion.Euler(-90f, yAngle + lookposion, 0f);
